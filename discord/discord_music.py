...
class DiscordBot(commands.Cog):
    ...
    async def empty_channel(self, ctx):
        if len(self.dictionary[ctx.guild.id]["voice_channel"].members) == 1:
            ...
            embed = discord.Embed(
                title="I've left the channel, because it was empty.", color=0x00FFCC, url="https://f.chulte.de"
            )
    ...
    async def next_song(self, ctx, function="next", url=None):
        ...
        if (not dictionary[ctx.guild.id]["voice_client"].is_playing()) and len(
            ...
            if (
                "title" in dictionary[ctx.guild.id]["song_queue"][0]
            ...
            try:
                ...
                self.log.warning(logging_manager.debug_info("song_loading_error " + str(e)))
                await dictionary[ctx.guild.id]["now_playing_message"].delete()
                embed = discord.Embed(
                    title="Error while loading... Trying once again...", url="https://f.chulte.de", color=0x00FFCC
                )
                await ctx.send(embed=embed)
                embed = discord.Embed(title="🔁 Loading ... 🔁", color=0x00FFCC, url="https://f.chulte.de")
                await ctx.send(embed=embed)
                small_dict = await self.youtube.youtube_term(small_dict["title"])
                if small_dict["error"] is True:
                    embed = discord.Embed(
                        title="Second Loading attempt also failed, try again with another searching term.",
            ...
            if ":" in str(self.dictionary[ctx.guild.id]["now_playing_song"]["duration"]):
                ...
                description = (
                    time.strftime("%H:%M:%S", time.gmtime(now_position))
                    + " / "
                    + str(self.dictionary[ctx.guild.id]["now_playing_song"]["duration"])
                )
            else:
            ...
            if not dictionary[ctx.guild.id]["voice_client"].is_playing():
                ...
                embed = discord.Embed(
                    title=":asterisk: Added **" + url + "** to Queue.", url="https://f.chulte.de", color=0x00FFCC
                )
                await ctx.send(embed=embed)
            ...
            embed = discord.Embed(
                title=":asterisk: Added " + str(length) + " Tracks to Queue. :asterisk:",
                url="https://f.chulte.de",
                color=0x00FFCC,
            )
            await ctx.send(embed=embed)
            ...
            if not self.dictionary[ctx.guild.id]["voice_client"].is_playing():
                ...
                embed = discord.Embed(
                    title=":asterisk: Added **" + track + "** to Queue.", url="https://f.chulte.de", color=0x00FFCC
                )
                await ctx.send(embed=embed)
            ...
            for track in tracks:
                ...
                dictionary[ctx.guild.id]["song_queue"].append(dic)
            embed = discord.Embed(
                title=":asterisk: Added " + str(len(tracks)) + " Tracks to Queue. :asterisk:",
                url="https://f.chulte.de",
                color=0x00FFCC,
            )
            await ctx.send(embed=embed)
            ...
            if not self.dictionary[ctx.guild.id]["voice_client"].is_playing():
                ...
                embed = discord.Embed(
                    title=":asterisk: Added **" + dic["title"] + "** to Queue.",
                    url="https://f.chulte.de",
                    color=0x00FFCC,
                )
                await ctx.send(embed=embed)
    ...
    async def play(self, ctx, *, url: str = None):
        if url is None:
            embed = discord.Embed(
                title="You need to enter something to play.", url="https://f.chulte.de", color=0x00FFCC
            )
            await ctx.send(embed=embed)
            return
        dictionary = self.dictionary
        try:
            if dictionary[ctx.guild.id]["voice_channel"] is None:
        ...
        try:
            if ctx.me.voice.channel != ctx.author.voice.channel:
                embed = discord.Embed(
                    title="You need to be in the same channel as the bot.", color=0x00FFCC, url="https://f.chulte.de"
                )
                await ctx.send(embed=embed)
                return
        ...
        if dictionary[ctx.guild.id]["voice_client"] is None:
            try:
                ...
                dictionary[ctx.guild.id]["voice_channel"] = None
                embed = discord.Embed(
                    title="Error while joining your channel. :frowning: (2)", url="https://f.chulte.de", color=0x00FFCC
                )
                await ctx.send(embed=embed)
        if "youtube" in url:
            if "/watch?v=" in url:
            ...
            if "playlist" in url:
                ...
                embed = discord.Embed(
                    title="This type of link is unsupported.", color=0x00FFCC, url="https://f.chulte.de"
                )
                await ctx.send(embed=embed)
        elif url == "charts":
            charts_url = (
                "https://open.spotify.com/user/spotifycharts/"
                "playlist/37i9dQZEVXbMDoHDwVN2tF?si=TTsODvC1S8uXfXtKq2VrRg"
            )
            await self.next_song(ctx, "sp_play", charts_url)
        else:
    ...
    async def cog_before_invoke(self, ctx):
        ...
        try:
            ...
            if self.dictionary[ctx.guild.id]["voice_client"] is None:
                self.dictionary[ctx.guild.id]["voice_client"] = ctx.guild.voice_client
        except AttributeError or discord.DiscordException or discord.HTTPException:
    ...
    async def rename(self, ctx, *, name: str):
        ...
        try:
            if len(name) > 32:
                embed = discord.Embed(
                    title="Name too long. 32 chars is the limit.", url="https://f.chulte.de", color=0x00FFCC
                )
                await ctx.send(embed=embed)
            me = ctx.guild.me
            await me.edit(nick=name)
            embed = discord.Embed(
                title="Rename to **" + name + "** successful.", url="https://f.chulte.de", color=0x00FFCC
            )
            await ctx.send(embed=embed)
        except Exception as e:
            embed = discord.Embed(title="An Error occurred: " + str(e), url="https://f.chulte.de", color=0x00FFCC)
            await ctx.send(embed=embed)
    ...
    async def volume(self, ctx, volume=None):
        ...
        if self.dictionary[ctx.guild.id]["voice_channel"] is None:
            embed = discord.Embed(title="The bot isn't connected.", color=0x00FFCC, url="https://f.chulte.de")
            await ctx.send(embed=embed)
            return
        ...
        if var < 0 or var > 2:
            embed = discord.Embed(
                title="The number needs to be between 0.0 and 2.0.", color=0x00FFCC, url="https://f.chulte.de"
            )
            await ctx.send(embed=embed)
            return
    ...
    async def info(self, ctx):
        ...
        try:
            embed = discord.Embed(
                title="Information",
                description="Name: "
                + str(dictionary[ctx.guild.id]["now_playing_song"]["title"])
                + "\nStreamed from: "
                + str(dictionary[ctx.guild.id]["now_playing_song"]["link"])
                + "\nDuration: "
                + str(dictionary[ctx.guild.id]["now_playing_song"]["duration"])
                + "\nRequested by: <@!"
                + str(dictionary[ctx.guild.id]["now_playing_song"]["user"].id)
                + ">\nLoaded in: "
                + str(round(dictionary[ctx.guild.id]["now_playing_song"]["loadtime"], 2))
                + " sec."
                + "\nSearched Term: "
                + str(dictionary[ctx.guild.id]["now_playing_song"]["term"]),
                color=0x00FFCC,
                url="https://f.chulte.de",
            )
            await ctx.send(embed=embed)
    ...
    async def quit(self, ctx):
        ...
        if self.dictionary[ctx.guild.id]["voice_client"] is not None:
            ...
            embed = discord.Embed(
                title="I need to be in a channel to leave! :thinking:", url="https://f.chulte.de", color=0x00FFCC
            )
            await ctx.send(embed=embed)
    ...
    async def clear(self, ctx):
        ...
        if len(self.dictionary[ctx.guild.id]["song_queue"]) is not 0:
            ...
            embed = discord.Embed(
                title="The Playlist was already empty! :cloud:", color=0x00FFCC, url="https://f.chulte.de"
            )
            await ctx.send(embed=embed)
    ...
    async def shuffle(self, ctx):
        ...
        if len(self.dictionary[ctx.guild.id]["song_queue"]) > 0:
            ...
            embed = discord.Embed(
                title="Shuffled! :twisted_rightwards_arrows:", color=0x00FFCC, url="https://f.chulte.de"
            )
            await ctx.send(embed=embed)
            await self.preload_next(ctx)
    ...
    async def stop(self, ctx):
        ...
        if dictionary[ctx.guild.id]["voice_client"] is not None:
            ...
            if (
                dictionary[ctx.guild.id]["voice_client"] is not None
            ...
            embed = discord.Embed(
                title=":thinking: The Bot isn't connected. :thinking:", color=0x00FFCC, url="https://f.chulte.de"
            )
            await ctx.send(embed=embed)
    ...
    async def pause(self, ctx):
        ...
        if dictionary[ctx.guild.id]["voice_client"] is not None:
            try:
                ...
                self.log.error(logging_manager.debug_info(str(e)))
                embed = discord.Embed(
                    title=":thinking: Nothing is playing... :thinking:", color=0x00FFCC, url="https://f.chulte.de"
                )
                await ctx.send(embed=embed)
        ...
        if dictionary[ctx.guild.id]["voice_client"] is not None:
            if dictionary[ctx.guild.id]["now_playing_song"] is not None:
                if count == 1:
                    ...
                    embed = discord.Embed(
                        title="Please provide a valid number.", url="https://f.chulte.de", color=0x00FFCC
                    )
                    await ctx.send(embed=embed)
                    return
                else:
                    if count > len(dictionary[ctx.guild.id]["song_queue"]):
                        embed = discord.Embed(
                            title="Skipped "
                            + str(len(dictionary[ctx.guild.id]["song_queue"]))
                            + " Tracks! :track_next:",
                            url="https://f.chulte.de",
                            color=0x00FFCC,
                        )
        ...
        if dictionary[ctx.guild.id]["voice_client"] is not None:
            try:
                if "pause_time" in dictionary[ctx.guild.id]["now_playing_song"]:
                    dictionary[ctx.guild.id]["now_playing_song"]["pause_duration"] += (
                ...
                embed = discord.Embed(
                    title=":thinking: Nothing is running... :thinking:", color=0x00FFCC, url="https://f.chulte.de"
                )
                await ctx.send(embed=embed)
    ...
    async def restart(self, ctx, restart_string=None):
        if restart_string is None:
            embed = discord.Embed(
                ...
                url="https://f.chulte.de/restart_token",
                color=0x00FFCC,
            )
            await ctx.send(embed=embed)
            return
        ...
        if restart_string == correct_string:
            embed = discord.Embed(title="Restarting!", url="https://f.chulte.de", color=0x00FFCC)
            await ctx.send(embed=embed)
            await self.bot.logout()
        else:
            embed = discord.Embed(title="Wrong token!", url="https://f.chulte.de", color=0x00FFCC)
            await ctx.send(embed=embed)
